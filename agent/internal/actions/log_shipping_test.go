...
func TestValidateLokiShipParams(t *testing.T) {
	cases := []struct {
		...
		{"valid ip", LokiShipParams{Host: "10.0.10.103", Port: "3100"}, true},
		{"valid host", LokiShipParams{Host: "loki.example.com", Port: "3100", Tenant: "team-a"}, true},
		{"missing port", LokiShipParams{Host: "10.0.10.103", Port: ""}, false},
		{"bad port", LokiShipParams{Host: "10.0.10.103", Port: "31x0"}, false},
		{"host injection space", LokiShipParams{Host: "10.0.10.103 evil", Port: "3100"}, false},
		{"host injection newline", LokiShipParams{Host: "10.0.10.103\n  tls: off", Port: "3100"}, false},
...
func TestBuildFluentBitConfig(t *testing.T) {
	cfg := buildFluentBitConfig(LokiShipParams{Host: "10.0.10.103", Port: "3100", TLS: false})
	for _, want := range []string{
		"name: systemd", "read_from_tail: on", "record_modifier",
		"Allowlist_key: MESSAGE", "name: loki",
		"host: 10.0.10.103", "port: 3100", "tls: off",
